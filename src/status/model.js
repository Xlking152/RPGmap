...
export const BUILTIN_STATUS_DEFINITIONS = deepFreeze([
  {
    ...
    maxStacks: 1,
    changes: [],
    capabilities: { collisionBypassGroups: ['structure'] },
    builtIn: true,
  },
  {
    id: 'status-rooted',
  ...
  },
  {
    ...
    maxStacks: 1,
    changes: [],
    capabilities: { canMove: false, canInteract: false, canActInCombat: false },
    builtIn: true,
  },
]);
...
function normalizeChanges(value, { token = false } = {}) {
  ...
  return value.slice(0, 16).flatMap(change => {
    ...
    const target = cleanText(change.target);
    ...
    return [{ target, mode, value: amount }];
  });
}
...
function normalizeCapabilities(value) {
  ...
  if (Array.isArray(source.collisionBypassGroups) && source.collisionBypassGroups.includes('structure')) {
    capabilities.collisionBypassGroups = ['structure'];
  }
  return capabilities;
}
...
export function normalizeStatusDefinition(value, { fallbackScopes = ['actor'], tokenOnly = false } = {}) {
  ...
  const name = cleanText(value.name || value.label, id, 120);
  const categoryValue = String(value.category || 'status');
  const category = STATUS_CATEGORIES.has(categoryValue) ? categoryValue : categoryValue === 'condition' ? 'status' : 'status';
  return {
    id,
    name,
...
function definitionView(definition) {
  return {
    ...clone(definition),
    label: cleanText(definition?.label || definition?.name, definition?.id || ''),
...
function legacyDefinition(effect, scope, forcedId = '') {
  ...
  const id = supplied && !BUILTIN_IDS.has(supplied)
    ? supplied
    : `status-legacy-${idSlug(name)}-${stableHash(semantic)}`;
...
function normalizeInstance(effect, { definition, scope, targetId, index, usedIds }) {
  const stacks = integer(effect?.stacks, 1, 1, Number(definition.maxStacks) || 1);
  const instance = {
  ...
  // Entity resolution in V1.5 still reads numeric changes from Actor effects.
  // Keeping this deterministic projection preserves those values while the
  // canonical source remains the World-level status definition.
  if (scope === 'actor' && definition.changes?.length) instance.changes = clone(definition.changes);
  return instance;
}

/**
 * Normalize the status-bearing portion of Entity System state. Legacy inline
 * Actor effects become a deterministic custom definition plus a lightweight
 * instance. The function is pure and is also used by the schema-v3 migration.
 */
export function normalizeEntityStatusState(raw) {
  ...
  const normalizeTargets = (targets, scope) => (Array.isArray(targets) ? targets : []).filter(Boolean).map(target => {
    const targetId = String(target.id ?? target.characterId ?? '');
    const usedIds = new Set();
    const byDefinition = new Map();
    const effects = [];
    const rawEffects = Array.isArray(target.effects)
      ? target.effects.filter(Boolean).slice(0, MAX_EFFECTS_PER_TARGET)
      : [];

    rawEffects.forEach((effect, index) => {
      ...
      if (!definition.scopes?.includes(scope)) {
        if (!definition.builtIn && (scope !== 'token' || !(definition.changes || []).length)) {
          ...
          if (!definitionsById.has(scoped.id)) {
            definitions.push(scoped);
            definitionsById.set(scoped.id, scoped);
          }
          definitionId = definition.id;
          migratedEffects += 1;
        }
      }
      if (scope === 'token' && (definition.changes || []).length) return;
      ...
      if (legacy) migratedEffects += 1;
    });

    return { ...clone(target), effects };
  });
  ...
  return {
    ...
    statusDefinitions: definitions.map(definition => clone(definition)),
    actors,
    tokens,
    migratedEffects,
  };
...
function targetContext(entityState, context = {}) {
  ...
  const tokenIdentity = context.tokenId ?? context.characterId ?? context.token?.id ?? context.token?.characterId;
  const token = context.token || (tokenIdentity == null ? null : tokens.find(item =>
    String(item?.id) === String(tokenIdentity) || String(item?.characterId) === String(tokenIdentity))) || null;
  const actorIdentity = context.actorId ?? context.actor?.id ?? token?.actorId;
  const actor = context.actor || (actorIdentity == null ? null : actors.find(item => String(item?.id) === String(actorIdentity))) || null;
  return { actor, token };
}
...
function resolvedInstance(instance, definition, scope, targetId) {
  return {
    ...
    stacks: integer(instance?.stacks, 1, 1, Number(definition.maxStacks) || 1),
...
function resolveTargetEffects(target, scope, definitionsById) {
  ...
  return (Array.isArray(target.effects) ? target.effects : []).flatMap(instance => {
    ...
    return [resolvedInstance(instance, definition, scope, target.id ?? target.characterId)];
  });
}
...
function resolveActorHealthForStatuses(actor, actorStatuses) {
  ...
  // A lightweight legacy Actor may not own the Health System fields yet. An
  // absent resource must not be interpreted as a zero-HP death.
...
export function resolveStatusCapabilities(statuses = []) {
  ...
  for (const status of enabled) {
    ...
    for (const group of status.capabilities?.collisionBypassGroups || []) {
...
export function resolveStatuses(rawEntityState, context = {}) {
  ...
  const statuses = [...actorStatuses, ...tokenStatuses, ...derivedStatuses]
  ...
  const statusVersion = stableHash({
    ...
    actorEffects: actor?.effects || [],
    tokenId: token?.id || null,
    tokenEffects: token?.effects || [],
    health: actor?.runtime?.health || null,
    hp: actor?.runtime?.resources?.hp || null,
    badStatuses: actor?.runtime?.badStatuses || null,
    currentFormId: actor?.currentFormId || null,
  });
  return {
    statuses,
    actorStatuses,
    tokenStatuses,
    derivedStatuses,
    capabilities: resolveStatusCapabilities(statuses),
    statusVersion,
  };
}

function statusError(message, code = 'invalid_status') {
...
function requiredId(value, label) {
  ...
  if (!result) throw statusError(`${label} is required`);
  return result;
}
...
function strictDefinition(value) {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(id)) {
    throw statusError('definition.id contains unsupported characters', 'invalid_status_id');
  }
  ...
  if (value.icon !== undefined && !STATUS_ICON_NAMES.has(cleanText(value.icon).toLowerCase())) {
    throw statusError('definition.icon is not an allowed Lucide icon', 'status_icon_invalid');
  }
  ...
  if (rawChanges.some(change => !plainObject(change) || !cleanText(change.target)
    || !STATUS_CHANGE_MODES.has(String(change.mode || 'add')) || !Number.isFinite(Number(change.value)))) {
    throw statusError('definition.changes contains an invalid change');
  }
  if (rawScopes.length === 1 && rawScopes[0] === 'token' && rawChanges.length) {
    throw statusError('Token statuses cannot modify Actor numeric values', 'status_scope_forbidden');
  }
  const requestedMax = integer(value.maxStacks, 1);
  if (requestedMax > 1 && rawChanges.some(change => String(change.mode || 'add') !== 'add')) {
    throw statusError('Stacking definitions may only use additive changes', 'status_stacking_invalid');
  }
  const allowedCapabilityKeys = new Set([...BOOLEAN_CAPABILITIES, 'collisionBypassGroups']);
  if (plainObject(value.capabilities)) {
    for (const key of Object.keys(value.capabilities)) {
      if (!allowedCapabilityKeys.has(key)) throw statusError(`definition.capabilities.${key} is not allowed`);
    }
    for (const key of BOOLEAN_CAPABILITIES) {
      if (value.capabilities[key] !== undefined && typeof value.capabilities[key] !== 'boolean') {
        throw statusError(`definition.capabilities.${key} must be boolean`);
      }
    }
    const bypass = value.capabilities.collisionBypassGroups;
    if (bypass !== undefined && (!Array.isArray(bypass) || bypass.some(group => group !== 'structure'))) {
      throw statusError('collisionBypassGroups may only contain structure');
    }
  }
...
function targetForOperation(entityState, message) {
  ...
  const entry = collection.find(item => String(item?.id) === targetId || (scope === 'token' && String(item?.characterId) === targetId));
  if (!entry) throw statusError(`${scope} target does not exist: ${targetId}`, 'status_target_not_found');
  entry.effects ||= [];
  return { scope, targetId: String(entry.id ?? targetId), target: entry };
}
...
function applySingleOperation(entityState, message, context) {
  ...
  if (type === 'status.definition.upsert') {
    ...
    if (index < 0) {
      ...
      const inUse = [...entityState.actors, ...entityState.tokens]
        .flatMap(target => target.effects || []).filter(effect => String(effect.definitionId) === definition.id);
      ...
      if (Math.max(1, ...inUse.map(effect => Number(effect.stacks) || 1)) > definition.maxStacks) {
        throw statusError('maxStacks is lower than an applied stack count', 'status_definition_in_use');
      }
      entityState.statusDefinitions[index] = definition;
      for (const actor of entityState.actors) {
        for (const effect of actor.effects || []) {
          if (String(effect.definitionId) === definition.id) {
            if (definition.changes.length) effect.changes = clone(definition.changes);
  ...
  if (type === 'status.definition.delete') {
    ...
    const referenced = [...entityState.actors, ...entityState.tokens]
      .some(target => (target.effects || []).some(effect => String(effect.definitionId) === definitionId));
  ...
  const definitionsById = new Map(getStatusDefinitions(entityState).map(definition => [definition.id, definition]));
  ...
  if (type === 'status.apply') {
    ...
    if (index >= 0) {
      ...
      const instance = {
        id: cleanText(message.instanceId) || (context.idFactory?.() || reducerId()),
        definitionId,
        stacks: Math.min(definition.maxStacks, amount),
        enabled: true,
  ...
  if (type === 'status.remove') {
    if (index >= 0) target.effects.splice(index, 1);
...
/** Apply one server-shaped status message to Entity System state atomically. */
export function reduceStatusOperation(rawEntityState, message, context = {}) {
  ...
  if (message?.type === 'status.batch') {
    ...
    results = message.operations.map(operation => {
      if (!['status.apply', 'status.remove', 'status.setStacks'].includes(String(operation?.type))) {
        throw statusError('status.batch only accepts apply/remove/setStacks operations');
      }
      return applySingleOperation(state, operation, context);
    });
  } else {
    results = [applySingleOperation(state, message, context)];
  }
...
export function statusStateFingerprint(rawEntityState) {
  ...
  const projection = {
    ...
    actors: state.actors.map(actor => ({
      ...
      currentFormId: actor.currentFormId,
      health: actor.runtime?.health || null,
      hp: actor.runtime?.resources?.hp || null,
    })),
    tokens: state.tokens.map(token => ({ id: token.id, characterId: token.characterId, actorId: token.actorId, effects: token.effects })),
  };
  return stableHash(projection);
}

export { MAX_STACKS, BUILTIN_IDS as BUILTIN_STATUS_IDS, STATUS_ICON_NAMES };
