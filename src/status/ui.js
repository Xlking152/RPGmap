...
export function renderStatusDefinitionEditor(definition = null) {
  ...
  const defaultDuration = current.defaultDuration || null;
  ...
  return `<div class="status-definition-backdrop" data-status-definition-editor><form class="status-definition-dialog" data-status-definition-form data-status-definition-id="${escapeStatusHtml(current.id || '')}" role="dialog" aria-modal="true">
    ...
    <div class="status-definition-grid"><label>稳定 ID<input name="id" maxlength="160" value="${escapeStatusHtml(current.id || '')}" ${isEdit ? 'readonly' : ''} required></label><label>显示名称<input name="name" maxlength="120" value="${escapeStatusHtml(current.label || current.name || '')}" required></label><label>分类<select name="category"><option value="neutral" ${category === 'neutral' ? 'selected' : ''}>中性</option><option value="buff" ${category === 'buff' ? 'selected' : ''}>Buff</option><option value="debuff" ${category === 'debuff' ? 'selected' : ''}>Debuff</option></select></label><label>Lucide 图标<select name="icon">${LUCIDE_STATUS_ICON_NAMES.map(name => `<option value="${name}" ${selectedIcon === name ? 'selected' : ''}>${name}</option>`).join('')}</select></label><label>颜色<input name="color" type="color" value="${safeColor(current.color)}"></label><label>最大叠加层数<input name="maxStacks" type="number" min="1" max="99" value="${Math.max(1, Number(current.maxStacks) || 1)}"></label><label>默认持续时间<select name="defaultDurationUnit"><option value="">永久</option><option value="turns" ${defaultDuration?.unit === 'turns' ? 'selected' : ''}>回合</option><option value="rounds" ${defaultDuration?.unit === 'rounds' ? 'selected' : ''}>轮</option></select></label><label>持续数量<input name="defaultDurationValue" type="number" min="1" max="10000" value="${defaultDuration?.value || ''}" placeholder="永久时留空"></label></div>
    ...
    <fieldset class="status-capability-grid"><legend>机械能力</legend><label>移动 ${capabilitySelect('canMove', capabilities.canMove == null ? '' : String(capabilities.canMove))}</label><label>互动 ${capabilitySelect('canInteract', capabilities.canInteract == null ? '' : String(capabilities.canInteract))}</label><label>战斗行动 ${capabilitySelect('canActInCombat', capabilities.canActInCombat == null ? '' : String(capabilities.canActInCombat))}</label><label>视野精度 <select name="visionPrecision"><option value="">继承默认</option><option value="vague" ${capabilities.visionPrecision === 'vague' ? 'selected' : ''}>仅模糊</option></select></label><label><input type="checkbox" name="collisionBypassStructure" ${capabilities.collisionBypassGroups?.includes?.('structure') ? 'checked' : ''}> 可穿越建筑阻挡</label></fieldset>
