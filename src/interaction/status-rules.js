...
export function evaluateFeatureStatusRule({ feature, action, tokenId = null, resolveStatus = null } = {}) {
  const rule = featureStatusRule(feature, action);
  let snapshot = null;
  if (tokenId && typeof resolveStatus === 'function') {
    snapshot = resolveStatus({ tokenId });
  ...
  if (!tokenId) return { ok: false, reason: '该操作需要先选择 Token 以检查状态', snapshot };
  if (!snapshot) return { ok: false, reason: '当前无法读取所选 Token 的状态', snapshot };
...
export function featureStatusMutations({ feature, action, state, tokenId, definitions = [] } = {}) {
  const rule = featureStatusRule(feature, action);
  ...
  const context = tokenContext(state, tokenId);
