...
} from '../engine/movement-distance.js';
...
function isMovementRoute(layer) {
  return layer instanceof L.Polyline
    ...
    && layer.options?.className === 'character-route-preview';
}
...
function createControl(mapElement, step) {
  ...
  control.title = '仅改变移动距离的显示/计价档位，不修改 A* 路径、碰撞或真实米数';
  const text = document.createElement('span');
  text.textContent = '移动档位';
  const select = document.createElement('select');
  select.setAttribute('aria-label', '移动距离显示档位');
...
function injectStyles(documentNode) {
  ...
  style.textContent = `
    .fvtt-distance-step-control {
      position: absolute; z-index: 1200; top: 12px; right: 12px;
      display: inline-flex; align-items: center; gap: 7px;
      padding: 6px 8px; border: 1px solid rgba(57,119,131,.42); border-radius: 7px;
      color: #33494d; background: rgba(248,250,247,.94); box-shadow: 0 2px 8px rgba(31,39,38,.13);
      font-size: 12px; font-weight: 700;
    }
    .fvtt-distance-step-control select {
      min-height: 27px; padding: 2px 22px 2px 6px; border: 1px solid #adb3af; border-radius: 5px;
      color: #252a2b; background: #fff; font: inherit;
    }
    .leaflet-tooltip.fvtt-segment-distance-label,
    .leaflet-tooltip.fvtt-distance-total-label {
      border: 0; box-shadow: 0 2px 7px rgba(0,0,0,.22); font-weight: 800; white-space: nowrap;
    }
    .leaflet-tooltip.fvtt-segment-distance-label {
      padding: 4px 6px; color: #fff; background: rgba(31,82,90,.90); font-size: 11px;
    }
    .leaflet-tooltip.fvtt-distance-total-label {
      padding: 5px 8px; color: #fff; background: rgba(150,63,47,.94); font-size: 12px;
    }
    .leaflet-tooltip.fvtt-segment-distance-label::before,
    .leaflet-tooltip.fvtt-distance-total-label::before { display: none; }
  `;
  documentNode.head.append(style);
}

export function createFvttSegmentDistanceTool({ defaultStep = 5 } = {}) {
  return {
    register(api) {
      ...
      let step = normalizeMovementDistanceStep(defaultStep);
      try {
        step = normalizeMovementDistanceStep(localStorage.getItem(storageKey), step);
      } catch {}
      ...
      function render() {
        ...
        const routePoints = flattenLatLngs(route.getLatLngs())
          .map(latlng => latLngToWorld(latlng, api.mapPackage.height));
        ...
        summary.segments.forEach((segment, index) => {
          ...
          L.tooltip({
            permanent: true,
            direction: 'center',
            className: 'fvtt-segment-distance-label',
            pane: 'measurePane',
            interactive: false,
          })
        ...
        L.tooltip({
          ...
          direction: 'bottom',
          offset: [0, 12],
          className: 'fvtt-distance-total-label',
          pane: 'measurePane',
          interactive: false,
        })
      ...
      control.select.addEventListener('change', () => {
        step = normalizeMovementDistanceStep(control.select.value, step);
