...
import { calculateWaypointRoute } from '../engine/movement-path.js';
import { TokenDragPhase, TokenDragPlan } from '../engine/token-drag.js';
...
export function createFvttWaypointMovementTool() {
  return {
    register(api) {
      ...
      async function calculate(point, { ready = false } = {}) {
        if (!drag.session || !pointInside(point, api.mapPackage)) return null;
        ...
        if (route.valid) {
          ...
          const action = drag.phase === TokenDragPhase.READY
            ? ' · 点击“确认移动”或按 Enter'
            : ' · Ctrl/Cmd+点击或 F 添加拐点';
          status('拖动路线 ' + formatDistance(route.distance) + ' · ' + drag.session.waypoints.length + ' 个拐点' + action);
        } else {
      ...
      function scheduleCalculate(point) {
        ...
        routeTimer = setTimeout(async () => {
          ...
          pendingPoint = null;
          if (target && drag.active && drag.phase !== TokenDragPhase.READY && drag.phase !== TokenDragPhase.MOVING) {
      ...
      async function addWaypointAtCurrent() {
        ...
        const route = drag.route?.valid ? drag.route : await calculate(drag.current);
      ...
      async function endTokenDrag(event) {
        ...
        const point = worldPointFromPointer(event);
        const route = drag.route?.valid && drag.current && Math.hypot(drag.current.x - point.x, drag.current.y - point.y) < 2
          ? drag.route
          : await calculate(point);
        if (!route?.valid) {
      ...
      async function planningClick(event) {
        if (!drag.active || moving || performance.now() < suppressClickUntil) {
      ...
      status('浏览模式：拖动地图；直接拖动角色 Token 可规划移动');
    }
  };
}
