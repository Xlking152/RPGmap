...
function installGhostStyles(documentNode) {
  if (documentNode.getElementById(GHOST_STYLE_ID)) return;
...
export function createMovementGhostRenderer() {
  return {
    register(api) {
      ...
      installGhostStyles(documentNode);
      const ghostLayer = L.layerGroup([], { pane:'characterPane' }).addTo(api.map);
      let selectedCharacterId = null;
      let ghostMarker = null;
      let endpointLayer = null;
      let hideTimer = null;
      const selectedCharacter = () => api.getState().characters?.find(character => character.id === selectedCharacterId) || null;
      ...
      const showGhost = layer => {
        ...
        if (!ghostMarker) {
          ghostMarker = L.marker(layer.getLatLng(), {
            icon:ghostIcon(descriptor), pane:'characterPane', interactive:false, keyboard:false, bubblingMouseEvents:false, zIndexOffset:900,
          }).addTo(ghostLayer);
        } else {
      ...
      api.on('character:select', event => { selectedCharacterId = event.detail?.id || null; });
      api.on('character:delete', event => {
        if (event.detail?.id === selectedCharacterId) { selectedCharacterId = null; hideGhost(); }
      });
      ...
      api.map.on('layerremove', event => { if (event.layer === endpointLayer) scheduleHide(); });
    },
  };
}
