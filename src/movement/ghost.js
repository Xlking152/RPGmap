...
export function createTokenGhostDescriptor(character, point, { blocked = false } = {}) {
  ...
  return Object.freeze({ characterId: character.id, name, initial: (name.trim()[0] || '?').toUpperCase(), avatarDataUrl, color: safeColor(character.color), point: copyPoint(point), blocked: Boolean(blocked) });
}
