...
test('map badges keep critical conditions first and collapse after four badges', () => {
  ...
  assert.ok(html.indexOf('title="死亡"') < html.indexOf('title="昏迷"'));
  assert.ok(html.indexOf('title="昏迷"') < html.indexOf('title="失能"'));
});
...
test('status writes stay pending until the confirmation Promise settles', async () => {
  ...
  const api = {
    ...
    status: {
      setStacks(payload) { calls.push(payload); return confirmation.promise; },
    },
  };
  const controller = createStatusUiController({ api, documentNode: {}, render: () => { renders += 1; } });
  const actionNode = {
    dataset: {
      statusAction: 'increment', statusScope: 'actor', statusTarget: 'actor-a',
      statusDefinition: 'poisoned', statusStacks: '1',
    },
  };
  const event = {
    preventDefault() {},
    target: { closest: selector => selector === '[data-status-action]' ? actionNode : null },
  };
...
test('OWNER and OBSERVER capabilities cannot mutate status data', () => {
  ...
  const actionNode = {
    dataset: { statusAction: 'remove', statusScope: 'actor', statusTarget: 'actor-a', statusDefinition: 'rooted' },
  };
...
test('actor status sheet exposes GM batch controls and a read-only player view', () => {
  const definitions = [
    { id: 'rooted', name: '定身', scopes: ['actor', 'token'], maxStacks: 1, builtIn: true },
  ];
  const api = {
    status: {
  ...
  const tokens = [
    { id: 'token-a', characterId: 'character-a', actorId: 'actor-a' },
    { id: 'token-b', characterId: 'character-b', actorId: 'actor-a' },
  ];
  const gm = renderActorStatusSheet({
    api, actor, tokens, allTokens: tokens, selectedTokenIds: ['character-a', 'character-b'], canManage: true,
  });
...
test('status UI is wired into Actor cards, sheet title, current inspector and app startup', () => {
  ...
  assert.match(entityUi, /api\.on\('status:change', \(\) => \{ renderPanel\(\); renderSheet\(\); \}\)/);
  assert.match(appShell, /class="ui-status-summary"/);
  assert.match(appShell, /api\.on\('status:change'/);
