...
test('movement snap steps cycle through 5/10/20/50/100 metres', () => {
  assert.deepEqual(MOVEMENT_STEPS, [5, 10, 20, 50, 100]);
  assert.equal(cycleMovementStep(20, -1), 10);
  assert.equal(cycleMovementStep(20, 1), 50);
  assert.equal(cycleMovementStep(5, -1), 5);
...
test('movement snapping affects only the live control point and preserves placed waypoints', () => {
  ...
  assert.deepEqual(session.current, { x: 80, y: 80 });
  ...
  assert.deepEqual(session.current, { x: 85, y: 75 });
  assert.deepEqual(session.waypoints, [{ x: 40, y: 20 }]);
});
