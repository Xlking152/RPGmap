...
function actor(id = 'actor-1', name = '测试角色') {
  return {
    id,
    name,
...
function legacyState() {
  return {
    ...
    characters: [{
      id: 'token-1', name: '测试角色', color: '#3d9b63', avatarDataUrl: null, visible: true,
      location: { type: 'map', x: 10.5, y: 20.5 },
    }],
    attackAreas: [],
    sceneEvents: [],
    preferences: {
      ...
      entitySystem: {
        ...
        tokens: [{
          ...
          rotation: 0, elevationFt: 5, hidden: false, locked: false, showName: true, effects: [],
        }],
      },
    },
  };
...
test('legacy single-map state migrates into World V2 with World Actors and Scene Tokens', () => {
  ...
  assert.equal(scene.mapPackage.id, 'test-map');
...
test('World V2 projects active Scene tokens back into the existing runtime character facade', () => {
  const state = legacyState();
  const world = createWorldV2FromRuntimeState(state, { mapPackage, ruleset });
  const scene = activeWorldScene(world);
  scene.tokens[0].x = 44.5;
  scene.tokens[0].y = 55.5;
  scene.tokens[0].hidden = true;
  const projected = projectWorldV2ToRuntimeState(state, world, { mapPackage, ruleset });
  assert.deepEqual(projected.characters[0].location, { type: 'map', x: 44.5, y: 55.5 });
  assert.equal(projected.characters[0].visible, false);
...
test('runtime movement updates only the active Scene while preserving other Scenes', () => {
  const state = legacyState();
  ...
  state.characters[0].location = { type: 'map', x: 99.5, y: 88.5 };
  const synced = synchronizeWorldV2FromRuntimeState(state, { mapPackage, ruleset, existingWorld: world });
  assert.equal(activeWorldScene(synced).tokens[0].x, 99.5);
  assert.equal(synced.scenes.find(scene => scene.id === 'scene-second').tokens.length, 0);
});

test('feature/building placement survives World V2 round-trip', () => {
  const state = legacyState();
  state.characters[0].location = { type: 'building', featureId: 'building-7' };
  const world = createWorldV2FromRuntimeState(state, { mapPackage, ruleset });
  const token = activeWorldScene(world).tokens[0];
  assert.equal(token.placement, 'feature');
  assert.equal(token.featureId, 'building-7');
  const projected = projectWorldV2ToRuntimeState(state, world, { mapPackage, ruleset });
  assert.deepEqual(projected.characters[0].location, { type: 'building', featureId: 'building-7' });
});
