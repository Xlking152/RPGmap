...
function edgePath() {
  ...
  for (const root of roots) {
    for (const suffix of [['Microsoft','Edge','Application','msedge.exe'], ['Microsoft','Edge SxS','Application','msedge.exe']]) {
...
const profile = await mkdtemp(path.join(os.tmpdir(), 'rpgmap-sheet-final-v2-'));
const edge = spawn(edgePath(), [
  '--headless=new','--disable-gpu','--disable-dev-shm-usage','--no-sandbox','--no-first-run','--no-default-browser-check',
  '--window-size=1440,1000', `--remote-debugging-port=${port}`, `--user-data-dir=${profile}`, targetUrl,
...
try {
  ...
  socket.addEventListener('message', event => {
    ...
    if (message.method === 'Runtime.consoleAPICalled' && message.params?.type === 'error') {
      exceptions.push((message.params.args || []).map(arg => arg.value ?? arg.description ?? '').filter(Boolean).join(' ') || 'browser console error');
    }
  });
  ...
  const ready = await retry(() => evaluate(`(() => {
    const api=document.querySelector('#app')?.rpgMapApp, mp=api?.multiplayer?.getStatus?.();
    ...
    return api?.world?.performOperations && api?.entities?.openActor && api?.entities?.openToken
      && mp?.connected===true && mp?.session?.role==='gm' && mp?.permissions?.worldWrite===true && /^联机同步完成/.test(status)
      ? { revision:mp.revision, status } : null;
  ...
  const snapshot = label => evaluate(`(() => {
    ...
    return {label:${JSON.stringify(label)},revision:api?.multiplayer?.getStatus?.()?.revision||null,records:api?.entities?.listOpenSheets?.()||[],
  ...
  const fixture = await evaluate(`(async()=>{
    const api=document.querySelector('#app').rpgMapApp;
    const existing=new Set((api.world.get().actors||[]).map(a=>String(a.id)));
    const resets=['${ids.pcActor}','${ids.npcActor}'].filter(id=>existing.has(id)).map(actorId=>({type:'actor.delete',payload:{actorId}}));
    if(resets.length) await api.world.performOperations(resets,{source:'smoke:final-v2:reset'});
    const imported=name=>({formName:'Smoke Form',identity:{name},description:{},resources:{hp:{max:20}},attributes:[],checks:{skills:[],saves:[]},badStatuses:[],combat:{attacks:[],defenses:[]},detection:{},tokenAppearance:{color:'#557f83',scale:1},source:{type:'manual'}});
    const pc0=api.ruleset.actor.createFromImport(imported('Smoke PC'),{actorId:'${ids.pcActor}',name:'Smoke PC'});
    const npc0=api.ruleset.actor.createFromImport(imported('Smoke NPC'),{actorId:'${ids.npcActor}',name:'Smoke NPC'});
    const pc={...pc0,id:'${ids.pcActor}',name:'Smoke PC',type:'pc',partyId:null,effects:[]};
    const npc={...npc0,id:'${ids.npcActor}',name:'Smoke NPC',type:'npc',partyId:null,effects:[]};
    const sceneId=api.world.get().activeSceneId;
    await api.world.performOperations([
      {type:'actor.upsert',payload:{actor:pc}},{type:'actor.upsert',payload:{actor:npc}},
    ...
    ],{source:'smoke:final-v2:setup'});
    const pcToken=api.tokens.get('${ids.pcToken}'), npcToken=api.tokens.get('${ids.npcToken}');
    const present=Boolean(pcToken&&npcToken);
    if(present){await api.entities.openToken('${ids.pcToken}');await api.entities.openToken('${ids.npcToken}');await api.entities.openActor('${ids.pcActor}');await api.entities.openActor('${ids.npcActor}');}
  ...
  const cards = await retrySnapshot(() => evaluate(`(()=>{
    ...
    if(r.npcToken.kind!=='npc'||r.npcToken.mode!=='instance'||r.npcToken.tab!=='overview')return null;
    if(r.pcActor.kind!=='character'||r.pcActor.interaction!=='play'||!r.pcActor.toggle)return null;
    if(r.npcActor.kind!=='npc'||r.npcActor.interaction!=='play'||!r.npcActor.toggle)return null;
    return r;
  })()`), 'Character/NPC live cards', deadline);

  const healthBefore = await retrySnapshot(() => evaluate(`(()=>{
    const api=document.querySelector('#app').rpgMapApp, t=api.health?.resolveToken?.('${ids.pcToken}'), a=api.health?.resolveActor?.('${ids.pcActor}');
    const input=document.querySelector('.entity-sheet[data-token-id="${ids.pcToken}"] [data-health-field-id]:not([disabled])');
    return t&&a&&input&&JSON.stringify(t)===JSON.stringify(a)?{health:t,fieldId:input.dataset.healthFieldId}:null;
  })()`), 'Linked PC Health shared before edit');
  const healthChange = await evaluate(`(()=>{
    const input=document.querySelector('.entity-sheet[data-token-id="${ids.pcToken}"] [data-health-field-id="${healthBefore.fieldId}"]:not([disabled])');
    const current=Number(input.value),min=input.min===''?0:Number(input.min);const next=current>min?current-1:current+1;
    input.value=String(next);input.dispatchEvent(new Event('change',{bubbles:true}));return{current,next};
  })()`);
  ...
  const resizeBefore = await evaluate(`(()=>{const pc=[...document.querySelectorAll('.entity-sheet[data-actor-id="${ids.pcActor}"]')].find(s=>!String(s.dataset.tokenId||'')),npc=[...document.querySelectorAll('.entity-sheet[data-actor-id="${ids.npcActor}"]')].find(s=>!String(s.dataset.tokenId||''));const a=pc.getBoundingClientRect(),b=npc.getBoundingClientRect();return{pc:{width:a.width,height:a.height},npc:{width:b.width,height:b.height}};})()`);
  const resizeApplied = await evaluate(`(()=>{
    const sheet=[...document.querySelectorAll('.entity-sheet[data-actor-id="${ids.pcActor}"]')].find(s=>!String(s.dataset.tokenId||''));
    const before=sheet.getBoundingClientRect(),targetWidth=Math.round(before.width+92),targetHeight=Math.max(320,Math.round(before.height-120));
    sheet.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,button:0,clientX:before.right-2,clientY:before.bottom-2}));
    sheet.style.width=targetWidth+'px';sheet.style.height=targetHeight+'px';
    const applied=sheet.getBoundingClientRect();
    sheet.dispatchEvent(new PointerEvent('pointerup',{bubbles:true,button:0,clientX:applied.right-2,clientY:applied.bottom-2}));
    return{requested:{width:targetWidth,height:targetHeight},actual:{width:applied.width,height:applied.height}};
  })()`);
  ...
  const resizeCaptured = await retrySnapshot(() => evaluate(`(()=>{
    const api=document.querySelector('#app').rpgMapApp,record=(api.entities.listOpenSheets?.()||[]).find(r=>r.key==='actor:${ids.pcActor}'),npc=(api.entities.listOpenSheets?.()||[]).find(r=>r.key==='actor:${ids.npcActor}');
    if(!record||Math.abs(Number(record.width)-${resizeApplied.actual.width})>2||Math.abs(Number(record.height)-${resizeApplied.actual.height})>2)return null;
    return{width:record.width,height:record.height,npcWidth:npc?.width??null,npcHeight:npc?.height??null};
  })()`), 'actual resize geometry captured by SheetManager');
