...
RPGmap 是一个面向桌面跑团的自托管 Web 战术地图工具。当前版本为 **2.5.2**，提供 World/Scene 管理、Actor 模板与 Token 实例、米制空间移动、生命/伤势、Status V4、战斗、折叠聊天、四级权限投影、三维感知与光源、战争迷雾、模板资料库和 Journal，以及 Windows 本机/局域网多人运行包。
...
2. 下载并解压 [RPGmap-v2.5.2.zip](https://github.com/Xlking152/RPGmap/releases/download/v2.5.2/RPGmap-v2.5.2.zip)。
...
- 地图工具：选择、框选、直接拖动、碰撞、测距和高度。普通拖放提交直线路线；拖动期间按住 `Ctrl`/`Cmd` 并松开鼠标进入分段规划，`Ctrl`/`Cmd`+点击或 `F` 连续添加拐点，普通点击或 `Enter` 提交，右键或 `Alt+F` 撤销，`Esc` 取消。规划线始终位于 Fog 上方，显示拐点、分段和累计距离。连续 WASD 合并为有界事务，群组拖动保持相对队形并原子提交。
