...
The ongoing refactor separates generic VTT capabilities from game-system rules and makes World/Scene/Token ownership explicit:
...
Movement and the visible Leaflet Token stack now consume active `Scene.tokens[]` through `api.tokens` rather than using `state.characters[]` as their source of truth. Token display data resolves through `api.tokens.resolveActor()`, so unlinked instances render their Synthetic Actor display state correctly.

The legacy Character projection is retained temporarily for the sidebar placement/editor UI and is scheduled for removal after that UI is migrated to Token APIs. See `文档/WORLD_V2.md` for the ownership model and migration sequence.
